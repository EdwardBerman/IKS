...
Perhaps the most amazing result in modern cosmology is that through studying correlations in shear, the subtle buldges in an objects observed ellipticity due to light bending around foreground objects, one can infer the distribution of dark matter in the universe. This paper presents the first forward model for weak lensing dark matter maps in Julia with IKS.jl. IKS.jl uses the iterative Kaiser-Squires method (sometimes referred to as KS$+$) to produce dark matter maps that are robust to data masks and edge effects. The paper outlines both the methods and implemtation details used by IKS.jl and compares the resulting dark matter maps with what is already found in the literature.

%This is a guide for authors who are preparing papers for JuliaCon using the \LaTeX{} document preparation system and the \verb|juliacon|  class file.

\end{abstract}

\section{Introduction}

\cite{remy2020probabilistic}
